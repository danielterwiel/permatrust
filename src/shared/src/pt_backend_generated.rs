...
#[derive(CandidType, Deserialize)]
pub enum WorkflowIdResult { Ok(WorkflowId), Err(AppError) }
...
#[derive(CandidType, Deserialize, Clone, Debug)]
pub struct WorkflowGraph { pub edges: Vec<Edge>, pub nodes: Vec<StateId> }
#[derive(CandidType, Deserialize, Clone, Debug)]
pub struct Workflow {
  pub id: WorkflowId,
...
#[derive(CandidType, Deserialize)]
pub enum WorkflowResult { Ok(Workflow), Err(AppError) }
#[derive(CandidType, Deserialize)]
pub enum DocumentsResult { Ok(Vec<Document>), Err(AppError) }
#[derive(CandidType, Deserialize, Clone, Debug)]
pub enum UserFilterField { FirstName, LastName }
#[derive(CandidType, Deserialize, Clone, Debug)]
pub enum DocumentFilterField { ProjectId, Version, Title, CreatedAt }
#[derive(CandidType, Deserialize, Clone, Debug)]
pub enum RevisionFilterField { ProjectId, Version, DocumentId, CreatedAt }
#[derive(CandidType, Deserialize, Clone, Debug)]
...
#[derive(CandidType, Deserialize, Clone, Debug)]
pub enum Entity { User, Document, Revision, Organisation, Project, Workflow }
#[derive(CandidType, Deserialize, Clone, Debug)]
pub enum FilterOperator { Contains, GreaterThan, LessThan, Equals }
#[derive(CandidType, Deserialize, Clone, Debug)]
...
#[derive(CandidType, Deserialize, Clone, Debug)]
pub struct SortCriteria { pub field: FilterField, pub order: SortOrder }
...
impl Service {
  ...
  pub async fn create_workflow(&self, arg0: CreateWorkflowInput) -> Result<(WorkflowIdResult,)> {
    ic_cdk::call(self.0, "create_workflow", (arg0,)).await
  }
  ...
  pub async fn get_workflow(&self, arg0: WorkflowId) -> Result<(WorkflowResult,)> {
    ic_cdk::call(self.0, "get_workflow", (arg0,)).await
  }
  ...
  pub async fn list_workflows(&self, arg0: PaginationInput) -> Result<(PaginatedWorkflowsResult,)> {
    ic_cdk::call(self.0, "list_workflows", (arg0,)).await
  }
}
