use crate::pt_backend_generated::{
    AppError, Document, FilterCriteria, Organisation, PageNumber, PageSize, PaginationMetadata,
    Project, Revision, SortCriteria, TotalItems, User, Workflow,
};
use crate::pt_backend_generated_traits::{Filterable, Sortable};
use candid::{self, CandidType, Deserialize};

pub type PaginatedUsersResult = PaginatedResult<User>;
pub type PaginatedWorkflowssResult = PaginatedResult<Workflow>;
...
pub fn paginate_doc<T: Clone + Filterable + Sortable>(
    items: &[T],
    page_size: PageSize,
