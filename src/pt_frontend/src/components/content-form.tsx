...
export const ContentForm: FC<ContentFormProps> = ({
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (value.uploaded_files && value.uploaded_files.length > 0) {
      ...
      if (value.markdown_content && value.markdown_content.trim()) {
        const originalMarkdownContent = originalRevisionContents?.find(content =>
          'Markdown' in content.content_type
        );

        // Check if markdown content is unchanged
        let isMarkdownUnchanged = false;
        if (originalMarkdownContent && originalMarkdownContent.content_data.length > 0) {
          try {
            ...
            console.warn('Could not compare markdown content, treating as changed:', error);
          }
        }
        ...
        if (isMarkdownUnchanged && originalMarkdownContent) {
          ...
          const markdownBytes = new TextEncoder().encode(value.markdown_content);
          contents.push({
      ...
      if (value.uploaded_files && value.uploaded_files.length > 0) {
        for (const file of value.uploaded_files) {
          ...
          const originalFileContent = originalRevisionContents?.find(content =>
          ...
          if (originalFileContent && originalFileContent.content_data.length > 0) {
            try {
              ...
              console.warn('Could not compare file content, treating as changed:', error);
            }
          }
      ...
      if (largeContent.length > 0) {
        ...
        console.log(`Will upload ${largeContent.length} large files in ${metrics.totalChunks} chunks (${(metrics.totalSize / 1024 / 1024).toFixed(2)}MB total)`);
        ...
        const largeFiles = value.uploaded_files.filter(file => file.size > MAX_DIRECT_UPLOAD_SIZE) || [];
  ...
  const handleFilesAccepted = useCallback((acceptedFiles: Array<FileWithPath>) => {
    form.setFieldValue('uploaded_files', (prev) => [...(prev || []), ...acceptedFiles]);
  }, [form]);
                ...
                >
                  <Dropzone.Zone className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-6 text-center hover:border-muted-foreground/50 transition-colors">
                    ...
                    <div className="flex flex-col items-center gap-2">
                      <Icon name="cloud-computing" size="lg" className="text-muted-foreground" />
                    ...
                    {(_files) => (
                      <div className="grid gap-2 w-full min-w-0">
                        {field.state.value.map((file, index) => {
                          const isLargeFile = file.size > MAX_DIRECT_UPLOAD_SIZE;
                          const fileKey = `${file.name}-${file.size}`;
                          const fileProgress = uploadProgress.fileProgress.get(fileKey);

                          return (
                            <div key={index} className="p-3 bg-muted rounded-lg space-y-2 w-full min-w-0">
                                ...
                                {fileProgress ? (
                                  <>
                                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                                      <span>
                                        {fileProgress.status === 'waiting' && 'Ready for upload'}
                                        {fileProgress.status === 'uploading' && (isLargeFile ? 'Uploading chunks...' : 'Uploading...')}
                                        {fileProgress.status === 'completed' && 'Upload complete'}
                                        {fileProgress.status === 'error' && 'Upload failed'}
                                      </span>
                                      <span>{fileProgress.percentComplete}%</span>
                                    </div>
                                    <Progress
                                      ...
                                      variant="secondary"
                                    />
                                    {isLargeFile && fileProgress.status === 'uploading' && (
                                      <div className="text-xs text-muted-foreground">
                                        Chunk {fileProgress.currentChunk} of {fileProgress.totalChunks}
                                      </div>
                                    )}
                                  </>
                                ) : (
      ...
      {(uploadProgress.isUploading || uploadProgress.percentComplete > 0) && (
        <div className="space-y-2">
          ...
          {uploadProgress.progress && (
            <div className="text-xs text-muted-foreground">
              Uploading chunk {uploadProgress.progress.chunkId + 1} of {uploadProgress.progress.totalChunks}
              {' '}({(uploadProgress.progress.uploadedBytes / 1024 / 1024).toFixed(1)}MB of {(uploadProgress.progress.totalContentSize / 1024 / 1024).toFixed(1)}MB)
            </div>
          )}
      ...
      <div className="flex justify-end">
        {isSubmitting || uploadProgress.isUploading ? (
          <Button disabled={true}>
            <Loading text={uploadProgress.isUploading ? 'Uploading...' : 'Saving...'} />
          </Button>
        ) : (
