...
type ExtendedHookOptions<TQueryFnData, TError, TData, TQueryKey extends QueryKey> = 
...
type ExtendedQueryOptions<TQueryFnData, TError, TData, TQueryKey extends QueryKey> = 
...
>(
  getQueryOptions: () => ExtendedQueryOptions<TQueryFnData, TError, TData, TQueryKey>
): (options?: ExtendedHookOptions<TQueryFnData, TError, TData, TQueryKey>) => UseQueryResult<TData, TError> {
  return function useQueryHook(options?: ExtendedHookOptions<TQueryFnData, TError, TData, TQueryKey>) {
    ...
    if (standardOptions && Object.keys(standardOptions).length > 0) {
    ...
    if (shouldUseDefaultErrorHandler) {
      ...
      const enhancedOptions = mergedOptions as unknown as { onError: typeof defaultOnError };
