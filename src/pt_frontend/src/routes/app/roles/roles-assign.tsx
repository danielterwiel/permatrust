...
export const Route = createFileRoute(
  ...
  loader: async ({ context, deps, params }) => {
    ...
    const [roles, usersResult] = await Promise.all([
      context.query.ensureQueryData(getProjectRolesOptions(projectId)),
      context.query.ensureQueryData(listUsersByProjectIdOptions(projectId)),
    ]);

    // Extract users array from the paginated result
    const users = usersResult[0]; // First element is the users array
