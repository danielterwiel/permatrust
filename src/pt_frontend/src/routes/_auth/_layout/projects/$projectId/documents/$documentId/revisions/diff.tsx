...
export const Route = createFileRoute('/_auth/_layout/projects/$projectId/documents/$documentId/revisions/diff')({
  ...
  loader: async ({ deps: { current, theirs } }) => {
    const revisions = await pt_backend.diff_document_revisions(BigInt(current), BigInt(theirs));
    return { revisions };
  },
});
...
function DocumentRevisionDiff() {
  ...
  useEffect(() => {
    try {
      const validatedRevisions = z.array(DocumentRevisionSchema).parse(revisions);
